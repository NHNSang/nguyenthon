...
const PostSidebarComponent: React.FC<SidebarComponentProps> = ({ relevantPost }) => {
    const handleClick = () => {
                ...
                <Card className="border-none shadow-md mb-8 ">
                    <CardContent className="p-6 bg-primary/20 rounded-md text-neutral-100">
                        <div className="flex flex-col items-center text-center">
                            ...
                            <Button
                                onClick={() => handleClick()}
                                className="w-full bg-primary hover:bg-primary/90 rounded-md text-lg">Nhắn tin facebook</Button>
                        </div>
                    </CardContent>
                </Card>
                    ...
                    <div className="space-y-4">
                        {relevantPost?.map((item) => (
                            <Link key={item.node.slug} href={`/du-an/${item.node.slug}`}>
                                <div className="group flex flex-col gap-4 items-start my-5">
                                    <div className="relative h-[150px] w-full flex-shrink-0 rounded-md overflow-hidden">
                                        <Image
                                    ...
                                    <div>
                                        <h4 className="font-medium group-hover:text-primary transition-colors line-clamp-2">{item.node.title}</h4>
                                        <div dangerouslySetInnerHTML={{ __html: item.node.excerpt || "" }} className="hidden text-sm text-gray-500" />
                                    </div>
                                </div>
                            </Link>
                        ))}
