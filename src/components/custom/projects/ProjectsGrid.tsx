...
const ProjectsGrid: React.FC<ProjectsGridProps> = ({
    ...
    const portfoliosByCategory = projects?.filter(project => project?.projectFields?.projectCategory[0] === seletedcategory)
        ...
        <section className=''>
            <Container>
                ...
                <motion.div
                    ...
                    whileInView="visible"
                    viewport={{ once: true }}
                    className='w-full overflow-x-scroll hiddenScrollBar mb-12'
                >
                    <div className='flex flex-row justify-center items-center gap-4 min-w-max px-4'>
                        <motion.button
                ...
                <AnimatePresence mode="wait">
                    <motion.div
                        ...
                        animate="show"
                        exit={{ opacity: 0 }}
                        className='grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-8 px-2 md:px-1 lg:px-0'
                    >
                        {(seletedcategory === "Tất cả" ? projects : portfoliosByCategory)?.map((project) => (
                            <motion.div
                                key={project.slug}
                                variants={item}
                                className='group'
                            >
                                <Link
                                    ...
                                    className='block rounded-lg overflow-hidden shadow-sm hover:shadow-xl transition-all duration-500'
                                >
                                    <div className='relative aspect-[4/3] overflow-hidden group'>
                                        ...
                                        >
                                            <ul className='text-white text-center space-y-2 transform translate-y-4 group-hover:translate-y-0 transition-transform duration-500'>
                                                <li className='font-medium'>Diện tích: {project.projectFields.sizeOfProject} m²</li>
                                                <li className='font-medium'>Quy mô: {project.projectFields.floor} tầng</li>
                                                <li className='font-medium'>Loại công trình: {project.projectFields.projectCategory[0]}</li>
                                            </ul>
                                        </motion.div>
                                    </div>
                                    <div className='p-4'>
                                        <h3 className='text-lg font-medium text-gray-800 group-hover:text-primary transition-colors duration-300 line-clamp-2 text-center'>
                                            {project.title}
                                        </h3>
                                    </div>
                                </Link>
