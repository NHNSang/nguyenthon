  ...
  try {
    ...
    return {
      ...
      openGraph: {
        ...
        type: 'article',
        images: validImageUrl ? [{ url: validImageUrl, width: 800, height: 600, alt: post?.title }] : [],
      },
    };
  } catch (error) {
...
export default async function SinglePostPage({ params }: { params: Promise<Params> }) {
  try {
    ...
    const PostsInSameCategory = response?.posts?.filter(
      (item) => item.node.categories.nodes[0]?.name === post.categories.nodes[0]?.name
    ) || [];
    const relevantPost = PostsInSameCategory.filter((item) => item.node.slug !== post.slug) || [];
            ...
            <div className="container">
              ...
              <div className="w-full">
                <div className="max-w-6xl line-clamp-2">
                  <h1 className="text-xl md:text-4xl font-bold tracking-tight mb-6">{post.title}</h1>
                </div>
                ...
                <div className="flex items-center gap-4">
                  <Avatar>
                    ...
                    />
                    <AvatarFallback>{post?.author?.node?.name || 'N/A'}</AvatarFallback>
                  </Avatar>
                  <div>
                    <p className="font-medium">{post?.author?.node?.name || 'Đang cập nhật'}</p>
                  </div>
                </div>
              </div>
          ...
          <section className="py-1">
            <div className="px-2 lg:container">
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
                {/* Main Content */}
                <ContentBlogPostProps headings={headings} updateHtml={updateHtml} post={post} />
