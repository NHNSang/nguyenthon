  ...
  try {
    ...
    return {
      ...
      description: res?.project?.excerpt,      openGraph: {
        ...
        type: 'article',
        images: validImageUrl ? [{ url: validImageUrl, width: 800, height: 600, alt: res?.project?.featuredImage?.node?.altText }] : [],
      },
    };
  } catch (error) {
...
export default async function SingleProjectPage({ params }: { params: Promise<Params> }) {
  try {
    ...
    const ProjectsInSameCategory = response?.projects?.nodes?.filter(
      (item) => item.projectFields.projectCategory[0] === project.projectFields.projectCategory[0]
    ) || [];
    const relevantProject = ProjectsInSameCategory.filter((item) => item.slug !== project.slug) || [];
              ...
              <div className="w-full">
                ...
                <div className="flex items-center gap-4">
                  <Avatar>
                    ...
                    />
                    <AvatarFallback>{project?.author?.node?.name || 'N/A'}</AvatarFallback>
                  </Avatar>
                  <div>
                    <p className="font-medium">{project?.author?.node?.name || 'Đang cập nhật'}</p>
                  </div>
                </div>
          ...
          <section className="py-4">
            <div className="px-2 lg:container">
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
                {/* Main Content */}
                <MainContentProjectPost project={project} updateHtml={updateHtml} headings={headings} />
